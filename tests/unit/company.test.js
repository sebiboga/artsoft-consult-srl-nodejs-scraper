...
const ARTSOFT_ANAF_RECORD = {
  ...
  address: 'STR. TEATRULUI, 9, Cluj-Napoca, Cluj',
  caenCode: '6201',
  inactive: false,
  registrationNumber: 'J2020012345678',
  vatRegistered: true,
...
describe('company.js', () => {
  ...
  describe('getCompanyBrand', () => {
    it('should return the company brand', () => {
      ...
      expect(brand).toBe('ARTSOFT CONSULT');
    });
  });

  describe('getCompanyData (no cache)', () => {
    it('should find Artsoft via ANAF search and return company data', async () => {
      mockFetch
    ...
    it('should throw when no active company found', async () => {
      mockFetch.mockResolvedValueOnce(anafSearchResponse([
        { cui: 11111111, name: 'ARTSOFT SOMETHING SRL', statusLabel: 'Radiată' }
      ]));
    ...
    it('should throw when ANAF returns no company name', async () => {
      mockFetch
        ...
        .mockResolvedValueOnce(anafCompanyResponse({ cui: 15997630, name: null }));
