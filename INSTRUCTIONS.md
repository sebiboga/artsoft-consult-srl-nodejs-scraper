...
8. **Scrape new jobs** - Extract jobs from ArtSoft Consult careers page (Romania)
...
3. **Scrape jobs** - Extract jobs from ArtSoft Consult careers page (Romania only)
...
company.js (validate company)
    ...
    │
    ▼ (if active)
scrape ArtSoft Consult HTML (jobs for Romania)
    │
    ▼
...
| `tests/validate-artsoft-consult-jobs.js` | CI fast validator (HEAD only); thin CLI over `src/job-validator.js` + `solr.js` |
