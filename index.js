...
const JOB_BASE = "https://www.artsoft-consult.ro";
...
function parseJobs(html) {
  ...
  $('div.single-job-container').each((i, el) => {
    const titleEl = $(el).find('h3 a');
    const job_title = titleEl.text().trim();
    const job_link = titleEl.attr('href');

    if (!job_title || !job_link) return;

    const url = job_link.startsWith('http') ? job_link : `${JOB_BASE}${job_link}`;

    jobs.push({
      url,
      title: job_title,
      company: COMPANY_NAME || 'ARTSOFT CONSULT SRL',
...
function mapToJobModel(job, cif, companyName) {
  const name = companyName || COMPANY_NAME || 'ARTSOFT CONSULT SRL';
  return {
    url: job.url,
    job_title: job.title,
    title: job.title,
...
async function scrapeAllListings() {
  const url = `${JOB_BASE}/careers/job-openings`;
...
async function main() {
  try {
    ...
    await addCompanyToCompanyCore(COMPANY_NAME, localCif, anafData, "https://www.artsoft-consult.ro/careers/job-openings");
